...
/***/ function(module, exports, __webpack_require__) {
	...
	(function () {
		'use strict';
		var screenOverlay = new _modulesScreenJs2['default'](),
		    navTrigger = (0, _vendorJquery214MinJs2['default'])('.primary-nav__trigger'),
					...
					for (var i = 0; i < data.length; i++) {
						...
						if (data[i].type !== null && data[i].type === 'link_set') {
							...
							for (var n = 0; n < data[i].content.length; n++) {
								(0, _vendorJquery214MinJs2['default'])('#interior__links_' + [i]).append('<a href=' + data[i].content[n].url + '>' + data[i].content[n].text + '</a>');
							}
						}
						if (data[i].type !== null && data[i].type === 'feature') {
							selected.next().append('<li>' + '<h4>' + data[i].name + '</h4>' + '<img src="' + data[i].content.url + '"/>' + '<p>' + data[i].content.text + '</p>' + '</li>');
						}
					}
				}
			});
